string = """13-16 k: kkkkkgmkbvkkrskhd
5-6 p: qpppvzp
3-4 p: psppxhlfpvkh
...
count = 0
for i,line1 in enumerate(string.split("\n")):
    string_to_check = line1.split(":")[1].strip()
    checker_string = line1.split(":")[0]
    letter_to_check = checker_string.split(" ")[1]
    lower_bound =  int(checker_string.split(" ")[0].split('-')[0])
    upper_bound =  int(checker_string.split(" ")[0].split('-')[1])

    indices = []

    for k,letter in enumerate(string_to_check):
        if letter==letter_to_check:
            indices.append(k+1)

    if (lower_bound in indices) ^ (upper_bound in indices):
        count += 1
        print(line1)
