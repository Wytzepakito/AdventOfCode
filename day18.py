...
def compute_string(string):
    ...
    while '*' in string or '+' in string:
        ...
        if '+' in string:
            ...
            comput_string = digits[index] + ' + ' + digits[index+1]
            result = add_string(comput_string)
            string = string.replace(comput_string, result)           
        elif '*' in string:
            index = operators.index('*')
            comput_string = digits[index] + ' * ' + digits[index+1]
            result = multiply_string(comput_string)
            string = string.replace(comput_string, result)
...
def remove_parenthesis(string):
    ...
    index = search.start()
    parenthesis = search.group()
    result = compute_string(parenthesis)
    result_string = string.replace(parenthesis, result)
