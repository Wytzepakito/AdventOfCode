string = """shiny purple bags contain 2 pale blue bags, 1 wavy fuchsia bag, 5 pale salmon bags.
...
string2 = """shiny gold bags contain 2 dark red bags.
...
for i, line in enumerate(string.split("\n")):
    ...
    list_bags  =re.findall(r'(?:\d )*[a-z]* [a-z]* bag[s ,.]',line)
    containing_bag =  " ".join(list_bags[0].split(" ")[:-1])
    if list_bags[1]=='no other bags':
        all_rules[containing_bag] = []
    else:
        all_rules[containing_bag] = list_bags[1:]


def get_gold_containing_bags(all_rules):
    shiny_gold_dic = {}

    added_bag = True
    while(added_bag==True):
        added_bag=False
        for rule in all_rules:
            containing_bag = " ".join(rule[0].split(" ")[:-1])
            for bag in rule[1:]:
                if 'shiny gold' in bag and containing_bag not in shiny_gold_dic.keys():
                    added_bag = True
                    print('HINY GOLD FOUNF')
                    print(rule)
                    match = re.search('\d', bag)
                    
                    if bool(match):
                        shiny_gold_dic[containing_bag] = int(match.group())
                    else:
                        shiny_gold_dic[containing_bag] = 1
                added_dic = {}
                for key, value in shiny_gold_dic.items():
                    if key in bag and containing_bag not in shiny_gold_dic.keys():
                        added_bag = True
                        match = re.search('\d', bag)
                        containing_bag = " ".join(rule[0].split(" ")[:-1])
                        if bool(match):
                            added_dic[containing_bag] = int(match.group())*value
                        else:
                            added_dic[containing_bag] = 1 * value
                shiny_gold_dic = {**shiny_gold_dic, **added_dic}
    return shiny_gold_dic
...
bag_multipliers = [1]
while (added_bag==True):
    print(bags_to_search)
    print(bag_multipliers)
    added_bag=False
    new_bags = []
    new_multipliers = []

    for i in range(len(bags_to_search)):
        bag = bags_to_search[i]
        bag_multiplier = bag_multipliers[i]
        lower_bags = all_rules[bag]
        grand_total+=bag_multiplier
        if len(lower_bags)==0:
            print(bag_multiplier)
            print(bag)
        for lower_bag in lower_bags:
            name =" ".join(lower_bag.split(" ")[1:3])
            multiplier = int(re.search('\d', lower_bag).group())*bag_multiplier
            added_bag = True
            new_bags.append(name)
            new_multipliers.append(multiplier)

    bags_to_search = new_bags
    bag_multipliers = new_multipliers
