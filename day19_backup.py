...
for line in string2.split("\n"):
    if line=="":
        ...
        rule = line.strip().split(":")[1].split(' ')
        rule.remove('')
        rules[index] = rule
    elif input_bool:
        inputs.append(line.strip())

def check_digits(rules, string):
    for i in string:
        if i.isdigit():
            if not(rules[i]==['"a"'] or rules[i]==['"b"']):
                return False
    return True

def is_unsettled(rules,digit_possibilities):
    for string in digit_possibilities:
        if check_digits(rules, string)==False:
            return True
    else:
        return False

def split_list(list1, char):
    master_list = []
    partial_list = []
    for item in list1:
        if item==char:
...
digit_posibilities = [rules['0']]



end_paths = []
finished_seqs = []
def make_strings(digit_posibilities, rules):
    made_strings = []

    while(is_unsettled(rules, digit_posibilities)):
        new_possiblities = []
        for digit_posibility in digit_posibilities:
            changed_number= False
            count = 0
            while(changed_number==False):
                char = digit_posibility[count]
                if char.isdigit():
                    #print(char)
                    #print(rules[char])
                    if '"' in rules[char][0]:
                        ...
                        new_possibilties = split_list(rules[char],"|")
                        for new_possibity in new_possibilties:
                            new_path = copy.deepcopy(digit_posibility)
                            new_path.pop(count)
                            for offset,digit in enumerate(new_possibity):
                                new_path.insert(count+offset,digit)
                           # print(new_path)
                            # check if path longer than max input
                            if check_digits(rules, new_path):
                                string =""
                                for char in new_path:
                                    string+= rules[char][0].strip().strip('"')
                                made_strings.append(string)
                                if string in inputs:
                                    end_paths.append(inputs.index(string))
                            else:
                                new_possiblities.append(new_path)
                            changed_number = True
                    else:
                        new_path = copy.deepcopy(digit_posibility)
                        new_path.pop(count)
                        for offset,digit in enumerate(rules[char]):
                            new_path.insert(count+offset, digit)
                        #print(new_path)
                            # check if path longer than max input
                        if check_digits(rules, new_path):
        ...
        print("NEW ITERATION")
        check= False
        digit_posibilities = new_possiblities

    return made_strings


#42
digit_posibilities_42 = [['53', '134'], ['116', '106']]
# 31
digit_posibilities_31 = [['11',]]

strings_42 = make_strings(digit_posibilities_42, rules) 

strings_31 = make_strings(digit_posibilities_31, rules)                       
...
##digit_posibilities.extend(finished_seqs)       
##strings = []    
##    
##for possiblity in digit_posibilities:
##    string =""
##    for char in possiblity:
##        string+= rules[char][0].strip().strip('"')
