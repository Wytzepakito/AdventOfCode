...
string2="""pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
def read_string(string):
    pass_port_items_list = []
    pass_port_items = []
    for line in string.split("\n"):
        if line == "":
            pass_port_items_list.append(pass_port_items)
            pass_port_items = []
        else:
            pass_port_items.extend(line.split(" "))
    pass_port_items_list.append(pass_port_items)
    return pass_port_items_list

def count_valid_entries(pass_port_items_list):

    count = 0
    for pass_port_items in pass_port_items_list:
        if len(pass_port_items)==8:
            count += 1
        if len(pass_port_items)==7 and 'cid' not in pass_port_items:
            count += 1
    return count

def check_items(pass_port_items):
    byr = False
    iyr = False
    eyr = False
    hgt = False
    ...
    if len(pass_port_items)>=7:
        for item in pass_port_items:
            ...
            if item.startswith('hcl'):
                colour = item.split(':')[1]
                hcl = bool(re.match(r'^#[0-9a-f]{6}', colour))

            if item.startswith('ecl'):
                colour = item.split(':')[1]
                options = ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']
                ecl = colour in options

            if item.startswith('pid'):
                id_num = item.split(':')[1]

                pid = bool(re.match(r'^[0-9]{9}', id_num))
    print("\n")
    print("pid, ecl, hcl, hgt, eyr, iyr, byr")
    print(pid, ecl, hcl,  hgt, eyr, iyr, byr)
    print(pass_port_items)
    print("\n")
    if (pid==True) and (ecl==True) and (hcl==True) and (hgt==True) and (eyr==True) and (iyr==True) and (byr==True):
        return True
    else:
        return False
...
def check_valid_passports(pass_port_items_list):
    count = 0
    for pass_port_items in pass_port_items_list:
        if check_items(pass_port_items):
            count += 1
...
pass_port_items_list = read_string(string)


count = check_valid_passports(pass_port_items_list)

print(count)

print(len(pass_port_items_list))           
